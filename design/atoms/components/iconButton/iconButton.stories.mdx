...
function SomeComponentWithSecondaryAction() {
  return (
    <>
      <IconButton size="icon-sm" icon={<Icon as={MagnifyingGlass} />} />
      <IconButton size="icon-md" icon={<Icon as={MagnifyingGlass} />} />
      <IconButton size="icon-lg" icon={<Icon as={MagnifyingGlass} />} />
    </>
  );
}
```
